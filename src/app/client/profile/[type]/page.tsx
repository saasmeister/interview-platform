...
export default function DocumentDetailPage() {
  ...
  if (!config) {
    return (
      <div className="py-20 text-center">
        <p className="text-muted-foreground">Onbekend document type</p>
      </div>
    );
  }
        ...
        <div className="flex flex-col items-center gap-3">
          <div className={`w-12 h-12 rounded-2xl ${config.bgColor} flex items-center justify-center`}>
            <span className="text-xl">
              {docType === "icp" ? "👤" : docType === "offer" ? "💰" : docType === "positioning" ? "🎯" : "🗣️"}
            </span>
          </div>
          <p className="text-sm text-slate-400">{config.title} laden...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="text-center py-16">
          ...
          <p className="text-muted-foreground max-w-md mx-auto">
            Dit document is nog niet gegenereerd. Rond het bijbehorende interview af of vraag je coach om een basis-document in te stellen.
          </p>
        </div>
      </div>
    );
  ...
  const pendingSuggestions = document.suggestions?.filter(
    (s) => s.status === "pending"
  ) ?? [];
        ...
        <Card className="shadow-sm">
          <CardContent className="p-8">
            <article className="prose prose-slate prose-sm max-w-none
              ...
              prose-strong:text-slate-900 prose-table:text-sm
              prose-th:bg-slate-50 prose-th:px-3 prose-th:py-2 prose-td:px-3 prose-td:py-2 prose-td:border-b
            ...
            {showSuggestionFull === suggestion.id ? (
              <div className="mb-4">
                ...
                <Button
                  ...
                  onClick={() => setShowSuggestionFull(null)}
                >
                  Inklappen
                </Button>
              </div>
